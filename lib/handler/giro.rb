  ...
  module Handler
    module Giro
      ...
      def transaction_exists? transaction
        `cat "#{ledger_file}" | grep -a5 ^#{transaction[:booking_date].strftime("%Y/%m/%d")} | grep -B5 -A2 "transaction_partner: #{transaction[:partner].strip.gsub('*', '\*')}" | grep "#{transaction[:description].strip.gsub('*', '\*')}"` != ""
      end
